...
def get_city_districts(city: str) -> dict:
    ...
    while True:
        ...
        districts = {}
        try:
            city_districts = response.json().get('suggestions', {})[0].get('entities')
...
def get_hotels(message: Message) -> dict:
    ...
    for hotel in hotels:
        ...
        if (hotel_name or address or dist or rating or star_rating or price) is not None and \
            ...
            total_price = str(
                (datetime.strptime(check_out, '%Y-%m-%d') - datetime.strptime(
                    check_in, '%Y-%m-%d')).days * int(price[1:]))
            ...
            info = (
                ...
                users_info_dict.get(message.from_user.id)[1]['city'], hotel_name, check_in,
                check_out, hotels_num, command, min_price, max_price, dist,
                'https://www.hotels.com/ho' + str(site), price, total_price)
            set_history(info)
    return hotels_info
